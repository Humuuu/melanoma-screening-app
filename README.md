...
Pastikan Python sudah terinstal di sistem Anda. Buka terminal/command prompt, arahkan ke folder proyek ini, dan jalankan:
```bash
pip install -r requirements.txt

**2. Catatan Khusus Pengguna macOS**
Jika saat menjalankan aplikasi muncul error terkait XGBoost seperti OpenMP runtime is not installed atau libxgboost.dylib could not be loaded, macOS Anda membutuhkan instalasi komponen sistem tambahan. Buka terminal dan jalankan perintah berikut menggunakan Homebrew:
```bash 
brew install libomp

**3. Menjalankan Antarmuka Web**
Setelah seluruh dependensi berhasil dipasang, jalankan perintah ini untuk membuka aplikasi di browser:
```bash 
